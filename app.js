...
function init() {
  ...
  (function initPremiumArcadeLab() {
    ...
    library.innerHTML = `
      ...
      <div class="arcade-profile-panel" id="premium-profile-panel" data-rank="c" aria-label="街机指挥档案">
        ...
        <div class="arcade-profile-grid">
          ...
          <span>成就 <strong id="premium-profile-achievements">0/40</strong></span>
      ...
      <div class="arcade-director-panel" id="premium-arcade-director" data-tone="daily" aria-label="街机导演推荐">
        ...
        <div class="arcade-director-meters" aria-label="街机完成度">
          ...
          <span>成就 <strong id="premium-director-achievements">0/40</strong></span>
        ...
        <div class="mini-game-panel" id="premium-heist" role="tabpanel" aria-labelledby="premium-tab-heist" tabindex="0" hidden>
          <div class="mini-game-copy">
            ...
            <div class="mini-stats">
              ...
              <span>幽影 <strong id="premium-heist-ghost">0%</strong></span>
            </div>
            <div class="mini-actions">
        ...
        <div class="mini-game-panel" id="premium-chain" role="tabpanel" aria-labelledby="premium-tab-chain" tabindex="0" hidden>
          <div class="mini-game-copy">
            ...
            <div class="mini-stats">
              ...
              <span>大师 <strong id="premium-chain-masterwork">0%</strong></span>
        ...
        <div class="mini-game-panel" id="premium-tactics" role="tabpanel" aria-labelledby="premium-tab-tactics" tabindex="0" hidden>
          <div class="mini-game-copy">
            ...
            <div class="mini-stats">
              ...
              <span>指令 <strong id="premium-tactics-command">0%</strong></span>
    ...
    const achievementDefs = [
      ...
      { id: 'heist_ghost_sweep', label: '幽影扫荡', desc: '赛博潜入中触发一次幽影扫荡' },
      ...
      { id: 'chain_masterwork', label: '大师炼成', desc: '连锁炼金触发一次大师炼成爆发' },
      ...
      { id: 'tactics_command', label: '指挥矩阵', desc: '裂隙战术中触发一次高级指挥矩阵' },
    ...
    function runDetailHighlights(game, details = {}) {
      const spec = {
        ...
        chain: [['movesLeft', '余步'], ['combo', '连锁'], ['masterworks', '大师'], ['mult', '倍率', 'x'], ['phase', '阶段'], ['recipes', '配方']],
        tactics: [['turns', '回合'], ['hp', '装甲'], ['kills', '击破'], ['commands', '指令'], ['combo', '连段', 'x'], ['counters', '反制'], ['surges', '脉冲']]
      }[game] || [];
      return spec
    ...
    function heistGhostState() {
      const heat = Math.round(clamp(Number(heist.ghostHeat || 0), 0, 100));
      const active = Math.max(0, Number(heist.ghostSweep || 0));
      const flash = Math.max(0, Number(heist.ghostSweepFlash || 0));
      return {
        heat,
        ready: heat >= 100,
        active,
        flash,
        sweeps: Number(heist.ghostSweeps || 0),
        label: active > 0 ? 'SWEEP' : flash > 0 ? 'FLASH' : heat >= 100 ? 'READY' : `${heat}%`,
        last: heist.lastGhostSweep || ''
      };
    }

    function triggerHeistGhostSweep(reason = 'combo') {
      if (heist.won || heist.locked || Number(heist.ghostSweep || 0) > 0) return false;
      const before = heistGhostState();
      const disabledCameras = heist.cameras
        .filter(camera => !camera.disabled)
        .sort((a, b) => heistDistance(a, heist.player) - heistDistance(b, heist.player))
        .slice(0, 2);
      disabledCameras.forEach(camera => {
        camera.disabled = true;
      });
      const distractedGuards = heist.guards
        .filter(guard => heistDistance(guard, heist.player) <= 7)
        .sort((a, b) => heistDistance(a, heist.player) - heistDistance(b, heist.player))
        .slice(0, 3);
      distractedGuards.forEach(guard => {
        guard.distracted = Math.max(Number(guard.distracted || 0), 5);
        guard.dir *= -1;
      });
      heist.ghostHeat = 0;
      heist.ghostSweep = 4;
      heist.ghostSweepFlash = 8;
      heist.ghostSweeps = Number(heist.ghostSweeps || 0) + 1;
      heist.lastGhostSweep = reason || 'combo';
      heist.cloakTurns = Math.max(Number(heist.cloakTurns || 0), 3);
      heist.security = Math.max(0, heist.security - (22 + Math.min(10, heist.chain)));
      heist.chain += 3 + disabledCameras.length + distractedGuards.length;
      heist.bestChain = Math.max(heist.bestChain, heist.chain);
      heist.loot += 110 + heist.chain * 9 + disabledCameras.length * 45 + distractedGuards.length * 20;
      heist.alert = 'GHOST';
      heist.lastTactic = 'GHOST SWEEP';
      heist.alarmFlash = Math.max(heist.alarmFlash || 0, 5);
      unlockAchievement('heist_ghost_sweep');
      setHeistUi();
      drawHeist();
      triggerPremiumFeedback('special', { label: 'GHOST SWEEP', throttleMs: 0 });
      return {
        triggered: true,
        before,
        after: heistGhostState(),
        disabledCameras: disabledCameras.length,
        distractedGuards: distractedGuards.length,
        reason: heist.lastGhostSweep
      };
    }

    function chargeHeistGhost(amount, reason = '') {
      if (heist.won || heist.locked || Number(amount || 0) <= 0) {
        return { charged: false, triggered: false, heat: heistGhostState().heat };
      }
      const before = heistGhostState().heat;
      heist.ghostHeat = clamp(before + Number(amount || 0), 0, 100);
      if (reason) heist.lastGhostSweep = reason;
      if (heist.ghostHeat >= 100 && Number(heist.ghostSweep || 0) <= 0) {
        const result = triggerHeistGhostSweep(reason);
        return { charged: true, triggered: !!result, before, heat: heistGhostState().heat, result };
      }
      return { charged: true, triggered: false, before, heat: heistGhostState().heat };
    }
    ...
    function setHeistUi() {
      ...
      const ghostEl = document.getElementById('premium-heist-ghost');
      if (ghostEl) {
        const ghost = heistGhostState();
        ghostEl.textContent = ghost.label;
        ghostEl.style.color = ghost.active > 0 || ghost.ready || ghost.flash > 0
          ? '#FDE68A'
          : ghost.heat >= 70
            ? '#A7F3D0'
            : '#94A3B8';
    ...
    function newHeist({ shouldFocus = true } = {}) {
      ...
      heist.ghostHeat = 0;
      heist.ghostSweep = 0;
      heist.ghostSweepFlash = 0;
      heist.ghostSweeps = 0;
      heist.lastGhostSweep = '';
      setHeistUi();
    ...
    function finishHeistHack() {
      ...
      const ghostResult = chargeHeistGhost(38 + hack.sequence.length * 4 + (camera ? 8 : 0), 'ZERO DAY');
      setHeistUi();
      drawHeist();
      if (!ghostResult.triggered) triggerPremiumFeedback('achievement', { label: 'ZERO DAY' });
      return true;
    }
    ...
    function heistAdvanceSensors() {
      ...
      heist.ghostSweepFlash = Math.max(0, Number(heist.ghostSweepFlash || 0) - 1);
      if (Number(heist.ghostSweep || 0) > 0) {
        heist.ghostSweep = Math.max(0, Number(heist.ghostSweep || 0) - 1);
        heist.cloakTurns = Math.max(Number(heist.cloakTurns || 0), 2);
        heist.security = Math.max(0, Number(heist.security || 0) - 0.9);
      }
    }
    ...
    function heistDebugState() {
      ...
      return {
        ...
        ghost: heistGhostState(),
        ghostHud: document.getElementById('premium-heist-ghost')?.textContent || '',
        chain: heist.chain,
    ...
    function triggerHeistTakedown() {
      ...
      const ghostResult = chargeHeistGhost(42 + (target.blindSide ? 12 : 0) + (heist.cloakTurns > 0 ? 8 : 0), target.blindSide ? 'SILENT TAKEDOWN' : 'GHOST TAKEDOWN');
      setHeistUi();
      drawHeist();
      if (!ghostResult.triggered) triggerPremiumFeedback('special', { label: 'SILENT TAKEDOWN', throttleMs: 120 });
      return {
        applied: true,
        ghost: ghostResult,
        before,
    ...
    function heistCaught(cause = 'HIGH') {
      ...
      heist.decoy = null;
      heist.hack = null;
      heist.ghostHeat = Math.max(0, Number(heist.ghostHeat || 0) - 28);
      heist.ghostSweep = 0;
    ...
    function finishHeistLockdown(cause = 'LOCKDOWN') {
      ...
      recordPremiumResult('heist', score, {
        ...
        hacksCompleted: heist.hacksCompleted,
        takedowns: Number(heist.takedowns || 0),
        ghostSweeps: Number(heist.ghostSweeps || 0),
        keys: heist.collected,
    ...
    function collectHeistTile(risk) {
      ...
      heist.keys = heist.keys.filter(k => {
        ...
        if (got) {
          ...
          ghostCharge += 14 + (heist.cloakTurns > 0 ? 12 : 0) + Math.min(10, Number(risk || 0) * 2);
          ghostReason = heist.cloakTurns > 0 ? 'GHOST KEY' : 'KEY';
        }
        return !got;
      });
      heist.caches.forEach(cache => {
        ...
        ghostCharge += 24 + Math.min(20, Math.floor(cache.value / 12)) + Math.min(10, Number(risk || 0) * 2);
        ghostReason = `CACHE ${cache.tier}`;
      ...
      if (ghostCharge > 0) chargeHeistGhost(ghostCharge, ghostReason);
    }
    ...
    function moveHeist(dx, dy) {
      ...
      if (guardCollision || guardSeen || (cameraSeen && heist.security >= 96)) {
        ...
        if (heist.cloakTurns > 0 && risk <= 1) {
          chargeHeistGhost(5, 'GHOST STEP');
        } else if (risk === 0 && heist.chain >= 6) {
          chargeHeistGhost(2, 'CLEAN ROUTE');
        }
    ...
    function drawHeist() {
      ...
      if (Number(heist.ghostSweepFlash || 0) > 0) {
        const alpha = clamp(Number(heist.ghostSweepFlash || 0) / 8, 0, 1);
        const px = heist.player.x * tile + tile / 2;
        const py = heist.player.y * tile + tile / 2;
        ctx.save();
        ctx.globalAlpha = 0.24 + alpha * 0.54;
        ctx.strokeStyle = '#FDE68A';
        ...
        ctx.shadowBlur = 22;
        ctx.beginPath();
        ctx.arc(px, py, 32 + (1 - alpha) * 34, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
        ctx.font = '900 10px JetBrains Mono, monospace';
        ctx.textAlign = 'center';
        ctx.fillStyle = '#FDE68A';
        ctx.fillText('GHOST SWEEP', px, Math.max(12, py - 42));
        ctx.restore();
      }
      ...
      const ghost = heistGhostState();
      ctx.fillStyle = ghost.active > 0 || ghost.flash > 0 || ghost.heat >= 70 ? '#FDE68A' : '#94A3B8';
      ctx.fillText(`GHOST ${ghost.label} · SWP ${ghost.sweeps}`, hudX + 10, 74);
      ctx.fillStyle = heist.hack?.active ? '#FDE68A' : '#94A3B8';
      ctx.fillText(`HACK ${heist.hack?.active ? heistProtocolText(heist.hack) : `DONE ${heist.hacksCompleted}`}`, hudX + 10, 90);
      ctx.fillStyle = 'rgba(255, 255, 255, 0.14)';
      ctx.fillRect(hudX + 10, 96, 214, 4);
      ctx.fillStyle = heist.security >= 82 ? '#EF4444' : heist.security >= 48 ? '#F59E0B' : '#34D399';
      ctx.fillRect(hudX + 10, 96, 214 * (heist.security / 100), 4);
      ctx.restore();
    ...
    function resetChainMasterwork() {
      chain.masterworkCharge = 0;
      chain.masterworks = 0;
      chain.bestMasterwork = 0;
      chain.lastMasterwork = null;
    }
    ...
    function formatChainMasterwork() {
      if (chain.lastMasterwork?.triggered) {
        return `MASTER +${chain.lastMasterwork.reward}`;
      }
      if (chain.bestMove?.masterwork?.ready) {
        return `PLAN ${chain.bestMove.masterwork.projected}%`;
      }
      return `${Math.floor(chain.masterworkCharge)}%`;
    }

    function chainProjectedRecipeCompletions(ledger = {}) {
      let index = chain.recipeIndex;
      let progress = { ...chain.recipeProgress };
      Object.keys(freshChainLedger()).forEach(key => {
        progress[key] = Number(progress[key] || 0) + Number(ledger[key] || 0);
      });
      let completed = 0;
      let guard = 0;
      while (guard < chainRecipeDefs.length) {
        const recipe = chainRecipeDefs[index % chainRecipeDefs.length];
        const done = Object.entries(recipe.req).every(([key, value]) => Number(progress[key] || 0) >= value);
        if (!done) break;
        const surplus = freshChainLedger();
        Object.keys(surplus).forEach(key => {
          surplus[key] = Math.max(0, Number(progress[key] || 0) - Number(recipe.req[key] || 0));
        });
        progress = surplus;
        index++;
        completed++;
    ...
    function chainMasterworkChargeFor(result = {}, ledger = {}, resonance = {}, recipeResult = {}, catalyst = false) {
      const completed = Array.isArray(recipeResult.completed) ? recipeResult.completed.length : Number(recipeResult.completed || 0);
      let charge = 0;
      if (resonance?.triggered) charge += resonance.tier === 'PRIME' ? 34 : 25;
      if (Number(result.cleared || 0) >= 10) charge += Math.min(28, 10 + (Number(result.cleared || 0) - 10) * 2);
      if (Number(result.specialsTriggered || 0) > 0) charge += Math.min(22, Number(result.specialsTriggered || 0) * 8);
      if (result.specialCreated) charge += result.specialCreated === 'prism' ? 18 : 12;
      if (completed > 0) charge += completed * 36;
      if (catalyst) charge += 18;
      if (Number(ledger.special || 0) >= 2) charge += 8;
      if (Number(result.cleared || 0) >= 18 && resonance?.triggered) charge += 12;
      return Math.round(clamp(charge, 0, 78));
    }

    function chainMasterworkPotential(result = {}, ledger = {}, resonance = {}) {
      const predictedRecipes = chainProjectedRecipeCompletions(ledger);
      const charge = chainMasterworkChargeFor(result, ledger, resonance, { completed: predictedRecipes }, !!result.catalyst);
      const projected = Math.min(100, Math.floor(chain.masterworkCharge + charge));
      return {
        charge,
        projected,
        ready: projected >= 100,
        recipes: predictedRecipes,
        label: projected >= 100 ? 'MASTER PLAN' : `${projected}%`
      };
    }

    function resolveChainMasterwork(result = {}, ledger = {}, resonance = {}, recipeResult = {}, catalyst = false) {
      const chargeEarned = chainMasterworkChargeFor(result, ledger, resonance, recipeResult, catalyst);
      chain.masterworkCharge = clamp(Number(chain.masterworkCharge || 0) + chargeEarned, 0, 140);
      if (chain.masterworkCharge < 100) {
        chain.lastMasterwork = {
          triggered: false,
          charge: Math.floor(chain.masterworkCharge),
          chargeEarned
        };
        return chain.lastMasterwork;
      }
      const recipeCount = Array.isArray(recipeResult.completed) ? recipeResult.completed.length : 0;
      const resonanceReward = resonance?.triggered ? Number(resonance.reward || 0) : 0;
      const reward = Math.round(
        520 +
        Number(result.cleared || 0) * 46 +
        Number(result.specialsTriggered || 0) * 140 +
        recipeCount * 260 +
        resonanceReward * 0.32 +
        (catalyst ? 260 : 0)
      );
      const overchargeBonus = catalyst ? 8 : 16;
      const moveRefund = Number(result.cleared || 0) >= 18 || (resonance?.triggered && recipeCount > 0) ? 1 : 0;
      chain.masterworkCharge = Math.max(0, chain.masterworkCharge - 100);
      chain.masterworks = Number(chain.masterworks || 0) + 1;
      chain.bestMasterwork = Math.max(Number(chain.bestMasterwork || 0), reward);
      chain.overcharge = clamp(Number(chain.overcharge || 0) + overchargeBonus, 0, 100);
      chain.moves = Math.min(36, Number(chain.moves || 0) + moveRefund);
      chain.mult = clamp(Number(chain.mult || 1) + 0.34, 1, 5);
      chain.lastMasterwork = {
        triggered: true,
        label: 'MASTERWORK',
        reward,
      ...
      unlockAchievement('chain_masterwork');
      triggerPremiumFeedback('special', { label: 'MASTERWORK', throttleMs: 180 });
      return chain.lastMasterwork;
    }
    ...
    function evaluateChainMove(r, c) {
      ...
      const ledger = collectChainLedger(move.cells);
      move.resonance = chainResonancePotential(ledger, move);
      move.masterwork = chainMasterworkPotential(move, ledger, move.resonance);
      return move;
    }
    ...
    function applyChainResult(result, { catalyst = false } = {}) {
      ...
      const masterwork = resolveChainMasterwork(result, ledger, resonance, recipeResult, catalyst);
      if (result.cleared >= 9) unlockAchievement('chain_combo_9');
      chain.lastGain = result.gain + resonance.reward + recipeResult.reward + Number(masterwork.reward || 0);
      ...
      const masterworkText = masterwork.triggered
        ? ` · 大师炼成 +${masterwork.reward}${masterwork.moveRefund ? ' · 返还 1 步' : ''}`
        : '';
      if (phase?.check(result)) {
        chain.score += phase.reward;
        chain.lastGain += phase.reward;
        chain.feedback = resonance.triggered
          ? `${phase.goal} 完成 +${phase.reward} · 共振 ${resonance.tier} +${resonance.reward}${masterworkText}`
          : `${phase.goal} 完成 +${phase.reward}${masterworkText}`;
        chain.phaseIndex++;
      } else if (recipeResult.completed.length) {
        chain.feedback = resonance.triggered
          ? `${recipeResult.completed.join('/')} 配方完成 +${recipeResult.reward} · 共振 ${resonance.tier} +${resonance.reward}${masterworkText}`
          : `${recipeResult.completed.join('/')} 配方完成 +${recipeResult.reward}${masterworkText}`;
      } else if (resonance.triggered) {
        chain.feedback = `共振 ${resonance.tier} ${resonance.detail} · +${resonance.reward}${masterworkText}`;
      } else if (masterwork.triggered) {
        chain.feedback = `大师炼成 ${result.cleared} 格 · +${masterwork.reward}${masterwork.moveRefund ? ' · 返还 1 步' : ''}`;
      } else if (catalyst) {
    ...
    function bestChainMove() {
      ...
      moves.sort((a, b) =>
        Number(b.masterwork?.ready) - Number(a.masterwork?.ready) ||
        Number(b.resonance?.triggered) - Number(a.resonance?.triggered) ||
        Number(b.masterwork?.charge || 0) - Number(a.masterwork?.charge || 0) ||
    ...
    function chainDebugState() {
      ...
      return {
        ...
        masterwork: {
          charge: Math.floor(chain.masterworkCharge),
          count: Number(chain.masterworks || 0),
          best: Number(chain.bestMasterwork || 0),
          last: chain.lastMasterwork ? { ...chain.lastMasterwork } : null,
          achieved: (career.achievements || []).includes('chain_masterwork'),
          hud: document.getElementById('premium-chain-masterwork')?.textContent || '',
          boardClass: chain.board?.className || '',
          detail: chain.board?.dataset.masterwork || ''
        },
        ...
        bestMove: best ? {
          ...
          masterwork: best.masterwork ? { ...best.masterwork } : null,
        ...
        hud: {
          ...
          masterwork: document.getElementById('premium-chain-masterwork')?.textContent || '',
    ...
    function seedChainMasterworkBoard() {
      switchPremiumGame('chain');
      seedChainResonanceBoard();
      chain.masterworkCharge = 88;
      chain.feedback = '大师预演矩阵已装载';
      chain.bestMove = evaluateChainMove(1, 1);
      chain.cursor = { r: 1, c: 1 };
      renderChain();
      return chainDebugState();
    ...
    function forceChainMasterwork() {
      const before = seedChainMasterworkBoard();
      popChain(1, 1);
      return {
        before,
        after: chainDebugState(),
    ...
    function renderChain() {
      ...
      const previewMasterwork = !!chain.bestMove?.masterwork?.ready;
      const phase = chainPhaseDef();
      ...
      chain.grid.forEach((row, r) => row.forEach((color, c) => {
        ...
        btn.className = `chain-cell chain-${color}${isHint ? ' chain-hint' : ''}${preview.has(key) && !isHint ? ' chain-preview' : ''}${previewResonance && preview.has(key) ? ' chain-resonance-plan' : ''}${previewMasterwork && preview.has(key) ? ' chain-masterwork-plan' : ''}${isCursor ? ' chain-cursor' : ''}${isWarning ? ' chain-warning-cell' : ''}`;
      ...
      document.getElementById('premium-chain-masterwork').textContent = formatChainMasterwork();
      ...
      chain.board.classList.toggle('chain-masterwork', !!chain.lastMasterwork?.triggered);
      chain.board.classList.toggle('chain-masterwork-ready', previewMasterwork && !chain.finished);
      ...
      chain.board.dataset.masterwork = formatChainMasterwork();
    ...
    function newTactics({ shouldFocus = false } = {}) {
      ...
      tactics.command = 0;
      tactics.commandBursts = 0;
      tactics.commandFlash = 0;
      tactics.commandLines = [];
      tactics.lastCommand = '';
      tactics.commandResolving = false;
      tactics.lastAction = '';
    ...
    function formatTacticsCommand() {
      if (tactics.lastCommand && Number(tactics.commandFlash || 0) > 0) return tactics.lastCommand;
      const value = Math.floor(Number(tactics.command || 0));
      const bursts = Number(tactics.commandBursts || 0);
      return bursts > 0 ? `${value}%/${bursts}` : `${value}%`;
    }

    function triggerTacticsCommand(reason = 'matrix') {
      if (tactics.won || tactics.lost) return null;
      const p = tactics.player;
      const before = {
        command: Math.floor(Number(tactics.command || 0)),
        bursts: Number(tactics.commandBursts || 0),
        ap: Number(p.ap || 0),
      ...
      const targets = livingTacticsEnemies()
        ...
        .filter(item => item.dist <= 6)
        .sort((a, b) => a.dist - b.dist || (b.enemy.type === 'warden') - (a.enemy.type === 'warden') || b.enemy.hp - a.enemy.hp)
        .slice(0, 3)
        .map(item => item.enemy);
      const from = tacticsCellCenter(p.x, p.y);
      const damage = 18 + Math.min(18, Number(tactics.momentum || 0) * 3);
      let killed = 0;
      let affected = [];
      tactics.commandResolving = true;
      try {
        affected = targets.map(enemy => {
          const targetBefore = { id: enemy.id, hp: enemy.hp, disrupted: enemy.disrupted || 0 };
          const result = damageTacticsEnemy(enemy, damage + (enemy.type === 'warden' ? 6 : 0));
          if (result.killed) killed++;
          else enemy.disrupted = Math.max(enemy.disrupted || 0, 2);
          return { ...targetBefore, afterHp: enemy.hp, afterDisrupted: enemy.disrupted || 0, killed: result.killed };
        });
      } finally {
        tactics.commandResolving = false;
      }
      tactics.commandLines = targets.map(enemy => ({ from, to: tacticsCellCenter(enemy.x, enemy.y), id: enemy.id }));
      tactics.commandBursts = Number(tactics.commandBursts || 0) + 1;
      tactics.commandFlash = 18;
      tactics.lastCommand = targets.length ? `MATRIX ${targets.length}T` : 'MATRIX SHIELD';
      p.ap = Math.min((p.baseAp || 3) + 2, Number(p.ap || 0) + 2);
      p.charge = Math.min(3, Number(p.charge || 0) + 1);
      p.shield = Math.min(48, Number(p.shield || 0) + 16 + targets.length * 3);
      tactics.suppression = Math.max(0, Number(tactics.suppression || 0) - 44);
      tactics.momentum = clamp(Number(tactics.momentum || 0) + 2 + targets.length, 0, 9);
      tactics.combo = Math.max(1, Number(tactics.combo || 0) + Math.max(1, targets.length));
      tactics.flash = Math.max(tactics.flash, 14);
      tactics.lastAction = 'command';
      tactics.message = targets.length
        ? `指挥矩阵展开：瘫痪 ${targets.length} 目标${killed ? ` · 击破 ${killed}` : ''}`
        : '指挥矩阵展开：行动返还与护盾重构';
      unlockAchievement('tactics_command');
      triggerPremiumFeedback('special', { label: 'COMMAND MATRIX' });
      return {
        triggered: true,
        reason,
        label: tactics.lastCommand,
        affected,
        killed,
        before,
        after: {
    ...
    function awardTacticsCommand(amount = 0, reason = '') {
      const gain = Math.max(0, Number(amount || 0));
      if (!gain || tactics.won || tactics.lost) return null;
      if (tactics.commandResolving) return null;
      const next = Number(tactics.command || 0) + gain;
      if (next < 100) {
        tactics.command = clamp(next, 0, 99);
        return null;
      }
      tactics.command = clamp(next - 100, 0, 99);
      return triggerTacticsCommand(reason || 'matrix');
    }
    ...
    function setTacticsUi() {
      ...
      const commandEl = document.getElementById('premium-tactics-command');
      if (commandEl) {
        const commandValue = Math.floor(Number(tactics.command || 0));
        commandEl.textContent = formatTacticsCommand();
        commandEl.style.color = tactics.commandFlash > 0 ? '#FDE68A' : commandValue >= 72 ? '#DDD6FE' : tactics.commandBursts > 0 ? '#A7F3D0' : '#94A3B8';
      }
    ...
    function finishTacticsWin() {
      ...
      const score = Math.max(250, 800 + tactics.player.hp * 8 + tactics.kills * 180 + tactics.player.cores * 260 + tactics.momentum * 45 + tactics.combo * 80 + tactics.surges * 120 + Number(tactics.commandBursts || 0) * 140 + Number(tactics.counters || 0) * 95 + Math.floor(tactics.surge) - tactics.turn * 22);
    ...
    function finishTacticsLoss(cause = 'MECH DOWN') {
      ...
      recordPremiumResult('tactics', score, {
        ...
        commands: Number(tactics.commandBursts || 0),
    ...
    function moveTactics(dx, dy) {
      ...
      if (enemy) {
        ...
        spendTacticsAp(1);
        if (!tactics.won && !tactics.lost) {
          awardTacticsCommand(result.killed ? 30 : 14, result.killed ? 'kill' : 'melee');
        }
        setTacticsUi();
        drawTactics();
        return;
      ...
      if (tactics.player.cores >= 3 && tactics.player.x === tactics.exit.x && tactics.player.y === tactics.exit.y) {
        ...
        spendTacticsAp(1);
        if (!tactics.won && !tactics.lost) {
          const commandGain = (collected ? 18 : 0)
            + (hazard?.counter?.triggered ? 18 : 0)
            + (followedRoute ? 10 : 0)
            + (cover.level > 0 ? 5 + cover.level * 3 : 0)
            + (hazard?.evaded ? 6 : 0)
            + (risk > 2 ? 3 : 0);
          const commandReason = collected ? 'core' : hazard?.counter?.triggered ? 'counter' : followedRoute ? 'route' : hazard?.evaded ? 'evasion' : 'advance';
          awardTacticsCommand(commandGain, commandReason);
        }
      }
      setTacticsUi();
      drawTactics();
    ...
    function triggerTacticsAction() {
      ...
      if (p.charge <= 0) {
        ...
        spendTacticsAp(1);
        if (!tactics.won && !tactics.lost) awardTacticsCommand(10 + cover.level * 5, 'guard');
        setTacticsUi();
        drawTactics();
      ...
      if (!targets.length) {
        ...
        commandGain = disrupted.length ? 18 + disrupted.length * 8 : 8;
        commandReason = disrupted.length ? 'jam' : 'shield';
        ...
        commandGain = 20 + hitTargets.length * 10 + killed * 16;
        commandReason = killed ? 'breach' : 'blast';
        tactics.message = `${targets.length > 1 ? '相位爆破贯穿双目标' : '相位爆破命中目标'}${refunded ? ' · 返还行动' : ''}`;
      }
      spendTacticsAp(1);
      if (!tactics.won && !tactics.lost) awardTacticsCommand(commandGain, commandReason);
      setTacticsUi();
      drawTactics();
    ...
    function drawTactics() {
      ...
      if (Number(tactics.commandFlash || 0) > 0) {
        const alpha = clamp(Number(tactics.commandFlash || 0) / 18, 0, 1);
        ctx.save();
        ctx.globalAlpha = 0.28 + alpha * 0.64;
        ctx.strokeStyle = '#DDD6FE';
        ctx.fillStyle = '#DDD6FE';
        ctx.lineWidth = 2 + alpha * 4;
        ctx.shadowColor = '#A78BFA';
        ctx.shadowBlur = 18;
        (tactics.commandLines || []).forEach(line => {
          ctx.beginPath();
          ctx.moveTo(line.from.x, line.from.y);
          ctx.lineTo(line.to.x, line.to.y);
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(line.to.x, line.to.y, 7 + (1 - alpha) * 10, 0, Math.PI * 2);
          ctx.stroke();
        });
        ctx.beginPath();
        const origin = tacticsCellCenter(tactics.player.x, tactics.player.y);
      ...
      ctx.fillStyle = '#fff';
      ctx.font = '800 12px JetBrains Mono, monospace';
      ctx.fillText(`SHIELD ${Math.round(p.shield)} · CHARGE ${p.charge} · MOM ${tactics.momentum} · COMBO ${tactics.combo}x · CNT ${Number(tactics.counters || 0)} · CMD ${formatTacticsCommand()} · SUP ${Math.floor(Number(tactics.suppression || 0))}%`, 18, 24);
      ...
      ctx.fillRect(18, 63, 118, 5);
      ctx.fillStyle = Number(tactics.commandFlash || 0) > 0 ? '#FDE68A' : '#DDD6FE';
      ctx.fillRect(18, 63, 118 * clamp(Number(tactics.command || 0) / 100, 0, 1), 5);
      ctx.fillStyle = Number(tactics.commandFlash || 0) > 0 ? '#FDE68A' : '#DDD6FE';
      ctx.font = '800 9px JetBrains Mono, monospace';
      ctx.fillText(`COMMAND ${formatTacticsCommand()}`, 144, 68);
      ctx.fillStyle = '#A78BFA';
    ...
    function tacticsDebugState() {
      ...
      return {
        ...
        surges: tactics.surges,
        command: {
          charge: Math.floor(Number(tactics.command || 0)),
          bursts: Number(tactics.commandBursts || 0),
          flash: Number(tactics.commandFlash || 0),
          label: tactics.lastCommand || '',
          lines: (tactics.commandLines || []).map(line => ({
            id: line.id || '',
            from: { ...line.from },
            to: { ...line.to }
          }))
        ...
        hud: {
          ...
          command: document.getElementById('premium-tactics-command')?.textContent || '',
    ...
    if (['127.0.0.1', 'localhost'].includes(window.location.hostname)) {
      window.__atherixDebug = {
        ...
        premium: {
          ...
          forceHeistGhostSweep: () => {
            switchPremiumGame('heist');
            newHeist();
            heist.player = { x: 6, y: 8 };
            heist.security = 68;
            heist.securityPeak = 68;
            heist.cloakTurns = 1;
            heist.chain = 8;
            heist.bestChain = 8;
            heist.loot = 180;
            ...
            heist.ghostSweepFlash = 0;
            heist.ghostSweeps = 0;
            heist.lastGhostSweep = 'DEBUG PRIME';
            heist.guards = [
              { x: 7, y: 8, dir: 1, axis: 'x', min: 5, max: 11, cone: 3, patrolAxis: 'x', distracted: 0 },
              { x: 10, y: 8, dir: -1, axis: 'x', min: 8, max: 12, cone: 3, patrolAxis: 'x', distracted: 0 },
              { x: 16, y: 7, dir: -1, axis: 'y', min: 3, max: 10, cone: 4, patrolAxis: 'y', distracted: 0 }
            ];
            heist.cameras.forEach(camera => {
            ...
            heist.alert = 'LOW';
            heist.lastTactic = '调试：幽影扫荡窗口';
            setHeistUi();
            drawHeist();
            const before = heistDebugState();
            const result = chargeHeistGhost(24, 'DEBUG SWEEP');
            setHeistUi();
            drawHeist();
            return {
              before,
              ...
              stageTone: document.getElementById('premium-game-stage')?.dataset.feedbackTone || '',
              stageLabel: document.getElementById('premium-game-stage')?.dataset.feedback || '',
              achieved: (career.achievements || []).includes('heist_ghost_sweep')
            };
          },
          forceHeistCache: () => {
            switchPremiumGame('heist');
          ...
          forceChainMasterwork: () => forceChainMasterwork(),
          ...
          tacticsForecast: () => {
            ...
            return {
              ...
              commandHud: document.getElementById('premium-tactics-command')?.textContent || '',
          ...
          forceTacticsCommand: () => {
            switchPremiumGame('tactics');
            newTactics({ shouldFocus: true });
            tactics.player = { ...tactics.player, x: 4, y: 3, hp: 92, shield: 6, ap: 1, baseAp: 3, charge: 1, cores: 1 };
            tactics.momentum = 5;
            tactics.combo = 2;
            tactics.surge = 44;
            tactics.surges = 0;
            ...
            tactics.commandLines = [];
            tactics.lastCommand = '';
            tactics.suppression = 78;
            tactics.enemies = [
              { id: 'drone-a', type: 'drone', x: 4, y: 1, hp: 34, maxHp: 45, disrupted: 0 },
              { id: 'turret-a', type: 'turret', x: 8, y: 3, hp: 60, maxHp: 60, disrupted: 0 },
              { id: 'hunter-a', type: 'hunter', x: 5, y: 5, hp: 52, maxHp: 70, disrupted: 0 },
              { id: 'warden-a', type: 'warden', x: 8, y: 6, hp: 95, maxHp: 95, disrupted: 0 }
            ];
            tactics.message = '调试：指挥矩阵待发';
            setTacticsUi();
            drawTactics();
            const before = tacticsDebugState();
            const result = awardTacticsCommand(12, 'debug');
            setTacticsUi();
            drawTactics();
            return {
              before,
              ...
              stageTone: document.getElementById('premium-game-stage')?.dataset.feedbackTone || '',
              stageLabel: document.getElementById('premium-game-stage')?.dataset.feedback || '',
              achieved: (career.achievements || []).includes('tactics_command')
            };
          },
          forceTacticsCounter: () => {
            switchPremiumGame('tactics');
