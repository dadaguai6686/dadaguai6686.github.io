...
```text
选择难度 -> 阅读本波事件与战术合约 -> 规划路线 -> 收集流明补电 -> 靠近信标修复 -> 避开风暴和碎片 -> 保持连锁 -> 完成合约或主目标 -> 进光门 -> 选择升级 -> 通关 5 波
```
...
- `src/game/simulation.ts`：可测试的游戏状态、难度、救援区域、波次事件、战术合约、路线教练、终局、计分、升级摘要、目标提示、资源告警、危险预警、成就挑战、结算评级、复盘统计和暂停逻辑
- `src/game/GameScene.ts`：Phaser 场景、程序化视觉、路线教练导航导引、维修光束、危险警戒、输入桥接、合约完成反馈和反馈事件
- `src/main.ts`：中文 HUD、战术合约面板、路线教练、资源告警、导航提示、当前改装、菜单成就墙、触控、暂停、成绩/成就存档、音效、结算评级、复盘和升级 UI
