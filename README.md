...
首屏会直接展示任务档案、胜利/失败条件和“补流明 → 修信标 → 撤离升级”的路线，第一次游玩推荐标准模式；想复盘同一张路线可以用救援代号、航行日志或今日挑战。
第一分钟路线固定写在开始按钮之后：读图补电、贴近维修、保命续分、撤离升级，让玩家不用猜颜色和目标。
...
- 暂停战术扫描：暂停/玩法说明会用更大的路线图复盘当前信标、补给和危险位置，手机端也能安全读图。
...
- 键盘输入保护：开始新局、重跑路线、暂停和继续时会重置 Phaser 键盘状态，避免按住移动、修复或推进进入菜单后恢复时残留旧输入。
...
- 局内任务条和导航提示：HUD 会把当前目标写成“当前任务/紧急任务/撤离任务”，提示下一步该补流明、修信标、脱离风暴或撤离，画布中会给目标加导引线和光环。
...
- `src/game/simulation.ts`：可测试的游戏状态、难度、救援区域、救援代号、波次事件、战术合约、合约焦点、开局读图缓冲、连锁窗口、路线教练、终局、计分、升级摘要、目标提示、资源告警、危险预警、成就挑战、实时评级、结算评级、复盘统计和暂停逻辑
- `src/game/GameScene.ts`：Phaser 场景、程序化视觉、中文物件标签、路线教练导航导引、合约目标标记、维修光束、危险警戒、可访问性渲染设置、输入桥接、合约完成反馈和反馈事件
- `src/main.ts`：中文 HUD、局内任务条、任务档案、战术合约面板、战场图例、战术雷达、暂停战术扫描、下一局目标看板、路线教练、实时评级、资源告警、导航提示、当前改装、菜单成就墙、今日挑战最佳、最近航行日志、触控、暂停/设置工具、可访问性设置、成绩/成就存档、音效、结算评级、复盘和升级 UI
