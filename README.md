...
发行复现使用 Node 22 和 `npm@11.16.0`，与 GitHub Pages 工作流保持一致。
...
开始按钮保持在操作细则、详细任务档案、图例和说明之前，方便手机玩家马上开局；按钮之后的任务资料、路线、图例、记录、成就、设置和详细操作默认折叠成资料抽屉，想读再展开。出航简报会用小地图和“安全读图、补给起手、按住维修、北侧撤离”四条中文作战令解释首局路线，让玩家不用猜颜色和目标。

## 游戏系统

- 中文主菜单、任务档案和局内玩法说明。
- 三档难度：练习、标准、硬核；硬核保留更高分数倍率和更紧的风险窗口，规则 smoke 和航点机器人会抽样检查路线可推进性，当前硬核样本要求首波、第 3 波和完整 5 波全通过，最终发行仍以本机浏览器验收和手感复核为准。
...
- 程序化资产层：无人机、信标、流明、碎片、风暴和光门带有驾驶舱、推进喷口、信标锁点、晶体切面、碎片裂纹、风暴旋臂和光门开闭 glyph，不再只是临时几何占位。
...
- 下一局目标看板：菜单和结算会根据当前存档、今日挑战、上一局表现和成就进度给出下一波/下一局目标，但默认收进“记录、成就与设置”抽屉，避免首屏像后台面板。
...
- 移动端战斗 HUD：手机游玩态会压缩评级、技能、合约和教练长文案，保留电量、机体、当前合约、路线教练短进度轨和触控操作；真实移动后仍保持短 HUD，不遮挡中心战场。
...
- 可读性增强：移动碎片会显示短轨迹和方向箭头，规则层也会按未来短轨迹提前标记 near/danger；靠近信标会显示维修光束，危险碎片靠近会出现警戒圈，低电量和低机体会触发 HUD 与无人机周围的告警效果。
- 状态化 HUD：菜单显示完整成绩入口但默认折叠，开局后自动收起品牌、长说明、记录条、常驻波次事件和改装列表，给移动、躲避和修复留出更多视野。
...
- 局内任务条和导航提示：HUD 会把当前目标写成“当前任务/紧急任务/撤离任务”，提示下一步该补流明、修信标、脱离风暴或撤离；关键转场会用“路线更新”短提示说明为什么目标改变，画布中会给目标加导引线、光环和短中文标签。默认开局路线只显示当前第一步文字，完整编号在“大字标签”模式中展开。
...
`npm test` 会跑规则层、中文 UI 静态检查、航点机器人 playtest 和发行 QA 文档门禁；`npm run qa:release` 会单独检查可提交的发行验收记录、QA 入口和发布口径；`npm run qa:browser-report` 会校验本机 Codex 内置浏览器原始报告里的场景覆盖、真实点击开局流程、过波升级链路探针、内置浏览器真实键盘/触控输入证据、移动端 HUD、真实触控移动后中心战场无遮挡、触控控件不遮挡、硬核文案、复盘计划、升级状态、运行时画面签名、生产资源哈希和本机输入探针证据；过波升级链路探针会用正式 `updateSimulation` 规则清完第 1 波，再在 Codex 内置浏览器 DOM 中点击升级卡并确认进入第 2 波；内置浏览器真实输入证据会从当前 Phaser 场景记录 `#lumen-browser-input-probes` 前后签名，确认 `source: codex-in-app-browser-real-input`、键盘输入或 `action: codex-in-app-browser-touch-stick-drag` 的触控摇杆拖动、读图缓冲结束、时间推进和玩家位置变化；`npm run build` 会类型检查、生产构建、按 package version/GitHub SHA 注入资源缓存版本，并同步根目录 `assets/` 兼容资源；`npm run test:dist` 会确认生产入口、GitHub 子路径资源解析、拆包资源、生产包体预算和根目录 `assets/` 文件哈希一致；`npm run qa:ship` 是本地发行总门禁，会把测试、构建、dist 检查和 Codex 内置浏览器报告校验串起来。GitHub Actions Pages 部署上传的是 `dist` artifact，生产资源使用相对路径，适配用户根站点、项目页和自定义域名；根目录 `assets/` 同步用于本地/分支 Pages 兼容、哈希校验和 CI 的 `git diff --exit-code -- assets` 陈旧资源检查。

发布前还要用 Codex 内置浏览器做一次玩家端验收：桌面 `1440x900`、手机竖屏 `390x844`、手机横屏 `844x390`，检查首屏中文四步说明、开始按钮、真实点击开局流程、内置浏览器真实键盘/触控输入、过波升级链路探针、手机横屏触控摇杆/动作按钮/暂停按钮不被 HUD 遮挡、暂停战术扫描、过波升级、通关结算、失败复盘和生产构建页面都能正常显示。为了稳定复现这些状态，本机 `localhost` / `127.0.0.1` 支持 `?qa=release&state=paused|won|completed|lost|playthrough` 验收入口；这个入口不会在 GitHub Pages 等非本机域名激活。当前 Pages loader 会按当前页面路径解析 `assets/`，可用于用户根站点、项目页或自定义域名部署。

最近一次 Codex 内置浏览器验收摘要保存在 `docs/release-qa.md`：开发版首屏三视口、真实点击开局流程、内置浏览器真实键盘输入、真实触控摇杆拖动、过波升级链路探针、暂停战术扫描、过波升级、通关结算、失败复盘和生产预览均通过。本机原始报告会写入 `output/qa/release-browser-qa-summary-current.json`，但 `output/` 是临时输出目录，不随仓库提交。按用户要求没有改用 Chrome 或桌面控制；该验收使用同一内置浏览器会话里的 DOM、布局矩形、触控目标、真实开局状态、`source: codex-in-app-browser-real-input` 的真实输入证据、过波升级链路探针、运行时画面签名证据、生产资源哈希和 `source: local-release-qa-simulation` 的本机输入探针证据。
